...
begin
  ...
  if (Qry_tmp.FieldByName('MM').asInteger>0) and (NrZajvCod>0) then
   begin
    NarCount:=Qry_tmp['MM'];
    Qry_tmp.Close;
    MyOpenSQL(Qry_narad,'select * from '+narTb+' where id_zav='+IntToStr(NrZajvCod)+
                      'order by nomer' );
    Qry_Narad.First;
    TbC_Nar.Tabs.Clear;
    for i:=0 to NarCount-1 do
      begin
        if i=0 then TbC_nar.Tabs.Add('Первичный')
...
//              Qry_runw.Locate('ID',Qry_narad['ID_WORK'],[]);
              ...
              Qry_loc.Locate('ID',Qry_narad['ID_LOCAL'],[]);
              Qry_Brig3.Locate('ID',Qry_narad['ID_BRIG2'],[]);
              mm_dopinf.Text:=Qry_narad.FieldByName('DOP_INF').asString;
              ...
              if Qry_Narad.FieldByName('ID_DISIN').asInteger=0 then
                 Lb_DisIn.Caption:=TrLangMSG(msgAdmin)
        else begin
              if (Qry_Brig2.Locate('ID',Qry_Narad.FieldByName('ID_DISIN').asInteger,[]))
                 then Lb_DisIn.Caption:=Qry_Brig2.FieldByName('NAME_R').asString;
             end;
          if (Qry_Narad.FieldByName('ID_DISOUT').asInteger=0) then
             Lb_DisOut.Caption:=TrLangMSG(msgAdmin)
          else begin
                if (Qry_Brig2.Locate('ID',Qry_Narad.FieldByName('ID_DISOUT').asInteger,[]))
                   then Lb_DisOut.Caption:=Qry_Brig2.FieldByName('NAME_R').asString;
               end;
...
//  DBL_runw.DisplayValue:=Qry_runw['NAME_R'];
  ...
  DBL_local.DisplayValue:=Qry_loc['NAME_R'];
...
{      tt_str:='insert into nnarad(id_zav,nomer,dop_inf,'+
      ...
      tt_str:='insert into ' + narTb + '(id_zav,nomer,dop_inf,'+
                      'dt_dep,dt_in,dt_out,id_brig,id_local,id_brig2,id_disin,id_disout,numbert,numteam) values('+IntToStr(NrZajvCod)+','+
      ...
      tt_str:=tt_str+Qry_BrIg.FieldByName('ID').asString+','+
                     Qry_loc.FieldByName('ID').asString+','+
...
//                     Qry_runw.FieldByName('ID').asString+','+
    ...
    begin
      tt_str:='update ' +  narTb + ' set '+
              ...
              'id_brig='+Qry_brig.FieldByName('ID').asString+
              ',id_local='+Qry_loc.FieldByName('ID').asString+
...
begin
  MyOpenSQL(Qry_tmp2,'select * from ' + narTb + ' where id='+inttostr(ss));
  Qry_tmp2.Edit;
  Qry_tmp2.FieldByName('DOP_INF').asString:=mm_DopInf.Text;
  Qry_tmp2.Post;
...
//  Label1.Caption:=TrLangMSG(msgDoWorkOne);
  ...
  Label10.Caption:=TrLangMSG(msgTypeLocalMany);
end;
