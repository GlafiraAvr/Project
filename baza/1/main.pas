...
//    procedure N29Click(Sender: TObject);
    ...
    procedure S_sah_miClick(Sender: TObject);
    ...
    procedure old_SI_univClick(Sender: TObject);
...
//
function ProvDate(date1,date2:TDateTime;time1,time2:string):boolean;
...
implementation
  ...
  FormZavFactory, S_Instrum, s_object, S_ObjObor;
{$R *.DFM}
...
begin
 ...
 if not assigned(S_WorkF) then
   S_WorkF:=TS_WorkF.CreateDic(application,'S_WORK', 'ID',DM1.S_Work_NoBlag_sur);
 Screen.Cursor:=crDefault;
...
begin
   ...
   tt_str:=VvodOl(Self,10,'DBN_AVAR',TrLangMSG(msgSelNoCloseZajav),
       ...
       ' id_ul2, kod_ul, dop_adr,dt_in,id_dopadres,id_object '+
       'from nzavjav z '+
       //
       'where '+_dop_where+
...
begin
  ...
  begin
    ...
    case OperAtt of
      toaVoda: Caption:=Caption+' ВОДОПРОВОД ';
      toaKanal: Caption:=Caption+' КАНАЛИЗАЦИЯ ';
      toaVRK: Caption:=Caption+' ВРК';
      toaObjVoda: Caption := Caption + ' ОБЪЕКТЫ ВОДОПРОВОДА';
      toaObjKanal: Caption := Caption + ' ОБЪЕКТЫ КАНАЛИЗАЦИИ';
    end;
  end;
   Fres.ShowModal;
end;
...
begin
 ...
 if Result then
   begin
      ...
      if date1<int(seg) then
         Result:=true
      else if date1=int(seg) then
        begin
...
//   Admm.ShowModal;
  inherited;
  Enabled := false;
  _AvrAdminManager := TAvrAdminManager.Create('AVARKREMEN', 'masterkey');
  try
...
procedure TFormMain.S_sah_miClick(Sender: TObject);
//var SpForm:TFormSprav;
//    sTit,sField:TstringList;
begin
...
(*  sTit:=TStringList.Create;
  ...
  sTit.Add('CONDITION');
  sField.Add(' ;WHERE ID>0; ');
  SpForm:=TFormSprav.SpravCreate(N31,'DBN_AVAR','s_sah',
  'Справочник колонок шахматки',sTit,sField);
  SpForm.ShowModal;
  *)
end;
...
procedure TFormMain.old_SI_univClick(Sender: TObject);
begin
  SmartPopup(N32Click,SI_univ);
end;
...
begin
  ...
  old_SI_zak.Caption:=TrLangMSG(msgClosed)+' заявки';
  old_SI_zak.Hint:=TrLangMSG(msgClosed)+' заявки|';
  old_SI_nzak.Caption:=TrLangMSG(msgNotClosed)+' заявки';
  old_SI_nzak.Hint:=TrLangMSG(msgNotClosed)+' заявки|';
  ...
  SI_zak.Hint:=TrLangMSG(msgClosed)+' заявки';
  SI_nzak.Hint:=TrLangMSG(msgNotClosed)+' заявки';
  ...
  SI_exit.Hint:=TrLangMSG(msgExit);
  SBM.panels[0].text:=TrLangMSG(msgVersion)+' '+TempVersion;
  NewZav_mi.Caption:=TrLangMSG(msgNewZajav);
  N23.Caption:=TrLangMSG(msgNotClosed)+' заявки';
  N24.Caption:=TrLangMSG(msgClosed)+' заявки';
...
begin
  ...
  //
    ...
    //DM1.Db_Avar.Close;
    //
    BRForm:=TBRForm.Create(nil,tamBackup,'AVARKREMEN');
    BRForm.ShowModal;
  finally
    FreeAndNil(BRForm);
    //
...
begin
  inherited;
  	try
    DM1.Db_Avar.Close;
    BRForm:=TBRForm.Create(nil,tamRestore,'AVARKREMEN');
    BRForm.ShowModal;
  finally
    FreeAndNil(BRForm);
    DM1.Db_Avar.Open;
  ...
  _dop_where:='(delz=1) and (id_attach='+IntToStr(Ord(OperAtt))+') ';
   tt_str:=VvodOl(Self,10,'DBN_AVAR',TrLangMSG(msgDelZajav),
      ////
       ...
       ' where (u.del in (''-'',''d'')) and (u.id_vidul>-2) and (u.id_vidul=v.id) '+
       '  and (u.id=z.id_ul1)) as ul1, '+
       ' id_ul2, kod_ul, dop_adr,dt_in, z.fyear, z.dt_in,id_dopadres,id_object '+
       'from nzavjav z '+
...
begin
  _Add( toaVoda,     'Водопровод' );
  _Add( toaKanal,    'Канализация' );
  _Add( toaVRK,      'Заявки ВРК' );
  _Add( toaObjVoda,  'Объекты водопровода' );
  _Add( toaObjKanal, 'Объекты канализации' );
...
begin
 ...
 if dm1.s_object_q.RecordCount > 0 then
 begin
    ...
    Application.MessageBox( 'Для редактирования данного справочника необходимо, ' + #13 +
        'чтобы справочник объектов содержал хотя бы одно значение', 'Внимание', MB_OK + MB_ICONWARNING  );
end;
