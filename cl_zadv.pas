...
//Qry_ul2.Close;
  if Qry_tadr['ID']=1 then
    begin
      lb_cross.Caption:='перекресток с';
...
begin
   ...
   if DataSet['UL_KOD']=0 then
    ...
    begin
      ...
      if pos('<пусто>', AnsiLowerCase(tt_str))=0 then ul:=trim(tt_str)
      else ul:='';
      tt_str:='Мест:'+ul;
...
//    Qry_bolt.Open;
  ...
  if (ZadvMode=0) or FReadOnly then
    begin
      ...
      begin
        if ((DE_Out.Date=NullDate) and  (Trim(DBL_brOut.DisplayValue)='< пусто >')) then
        begin
          DE_Out.Enabled:=true;
          Te_Out.Enabled:=true;
        ...
        if ((DE_Gran.Date=NullDate) and  (Trim(DBL_brGran.DisplayValue)='< пусто >')) then
        begin
        ...
        if ((DE_Rec.Date=NullDate) and  (Trim(DBL_brRec.DisplayValue)='< пусто >')) and (zAttach in [toaVoda,toaKanal,toaVRK,toaObjVoda,toaObjKanal]) then
        begin
          DE_Rec.Enabled:=true;
          TE_Rec.Enabled:=true;
...
//             Qry_bolt.Locate('ID',Qry_zadv['ID_BOLT'],[]);
  ...
  begin
    ...
    begin
     //
      Label13.Caption := 'Перекрыта';
      Label14.Caption := 'Открыта';
      BB_Open.Caption := 'Открыть';
    end
    else
    begin
      //
      Label13.Caption := 'Открыта';
      Label14.Caption := 'Перекрыта';
      BB_Open.Caption := 'Перекрыть';
    end;
