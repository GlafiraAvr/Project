...
begin
  ...
  sbar.SimpleText:='Размер изображения: '+IntToStr(Image.Picture.Width)+'x'+IntToStr(Image.Picture.Height);
  Caption:='Изображения аварии - '+'изображение №'+IntToStr(ImageIndex+1)+' из '+IntToStr(F_FileList.Count);
end;
...
begin
  if Application.MessageBox('Вы уверены, что хотите удалить изображение?',
        'Диспетчер АВР', MB_YESNO+MB_ICONQUESTION)<>IDYES	then exit;
