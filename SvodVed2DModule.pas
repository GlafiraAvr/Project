...
// if not F_GroupByState then exit; // только для СводВед2, а для СводВед21 - не надо
  ...
  if zav_type_name = 'ВЫПОЛНЕНО' then
            dset_des.FieldByName('vipol').AsString:=DeletePusto(timetostr( _time));
...
begin
  ...
  begin
    md_res.Append;
    insert_base_field('ЗАПЛАНИРОВАННЫЕ', dset_tmp, md_res);
...
begin
  ...
  MyOpenIBDS(dset_tmp, Format(F_SEL_ZAV, ['zavjav',
                                          ord(OperAtt),
  ...
  begin
    md_res.Append;
    insert_base_field('ВЫПОЛНЕНО', dset_tmp, md_res);
    md_res.Post;
    dset_tmp.Next;
  end;
end;
...
begin
  Result:=dt_field+'>"'+DateToStr(F_dt_smena-1)+' '+TimeToStr(F_tm_smena)+'"'+
          ' and '+dt_field+'<="'+DateToStr(F_dt_smena)+' '+TimeToStr(F_tm_smena)+'"';
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (dttm>dttm_smena-1) and (dttm<=dttm_smena) then
        begin
          is_find:=true;
          break;
        end;
...
//        insert_base_field('В РАБОТЕ НЕЗАПЛАНИРОВАННЫЕ', dset_tmp, md_res);
        insert_base_field('В РАБОТЕ', dset_tmp, md_res);
        md_res.Post;
      end;
