...
begin
  ...
  begin
    _Error( 'Введите тип адреса!', rfUlKod );
    exit;
  end;
  ...
  then
  begin
    _Error( 'Введите наименование улицы!', rfUl1 );
    exit;
  end;
  ...
  case dset_Raskop.FieldByName( 'UL_KOD' ).AsInteger of
    0: //Улица, номер дома
      ...
      begin
        _Error( 'Введите номер дома!', rfHouse );
        exit;
      end;
    1: //Перекресток
      if StrToInt( trim( dset_Raskop.FieldByName( 'ID_UL2' ).AsString ) ) < 1 then
      begin
        _Error( 'Введите наименование улицы №2!', rfUl2 );
        exit;
      end;
  end;
  ...
  begin
    _Error( 'Введите местность повреждения!', rfMest );
    exit;
  end;
...
begin
  ...
  if _IsErr then
    raise Exception.Create( 'Имеется активная транзакция ( Tdm_NGRaskop.DataModuleDestroy )' );
end;
