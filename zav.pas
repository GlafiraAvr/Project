...
type
  ...
  TFormZav = class(TOperAttForm)
    ...
    procedure DBL_sodChange(Sender: TObject);
...
begin
  ...
  if (not(FDM_Zav.Qry_owner.FieldByName('OWNER_TYPE').asInteger in [3,4]{предварительная,ложная})) and (FNNarCount=0)
   ...
   then
    begin
      MessageBox(handle,PChar('Заполните первичный выезд!'),
...
begin
  if ((Lb_Nzav.Caption<>'Новый') and value) or not value then
    begin
            ...
            /////////////////
              {5}try
                ...
                {6}try
                  ...
                  inttostr(strtointdef(FDM_Zav.Qry_sod.FieldByName('ID').asString,-1))+','+
                  '-1,';
           ...
           begin
             {11}try
               ...
               { ',Utoch_Xar_Povr='+QuotedStr(FUtoch_Xar_Povr)+}
                ...
                {/01.10.2004}
                ', id_sod='+inttostr(strtointdef(FDM_Zav.Qry_sod.FieldByName('ID').asString,-1))+
...
//  Label10.Caption:=TrLangMSG(msgContext);
...
//  Label5.Caption:=TrLangMSG(msgSpotPlace);
//  Diam_lb.Caption:=TrLangMSG(msgDiam);
...
//  Label19.Caption:=TrLangMSG(msgTypeNetOne);
//  Label6.Caption:=TrLangMSG(msgTypeLeakOne);
//  Label26.Caption:=TrLangMSG(msgContext);
//  Label17.Caption:=TrLangMSG(msgTypeClogOne);
//  Label18.Caption:=TrLangMSG(msgTypeFillOne);
//  Label20.Caption:=TrLangMSG(msgPlaceSHIT);
...
procedure TFormZav.DBL_sodChange(Sender: TObject);
begin
  inherited;
  UpdateSod;
  DBL_raonChange(Self);
...
var s:string;
begin
  if CB_DelZayav.Checked then s:='убрать из нарядов на удаление'
    else s:='поместить на удаление';
  if MessageBox(handle,PChar('наряд '+s+'?'),'Диспетчер АВР',MB_OKCANCEL or MB_ICONINFORMATION)=ID_CANCEL
    then Key:=0;
  inherited;
end;
...
begin
  ...
  if not Detect_FirstStrOfDopInfOnDelStatus then
    begin
      result:=true;
      edDop_Inf.Lines.Insert(0,DEL_STATUS_KEYWORD+' поставлена на удаление '
          ...
          begin
            if (Trim(edDop_Inf.Text)<>'') then
              begin
                ...
                begin
                  MessageBox(handle,PChar('Дополнительная информация не заполнена!'+#13+
                    'Заполните ее причиной постановки на удаление'),'Диспетчер АВР',MB_OK or MB_ICONINFORMATION);
...
begin
  ...
  if DBL_Mest.LookupSource.DataSet.FieldByName('VOSST_POKRYT').AsString='ВОССТ.' then
    Result := 1   //сооствет. "ВОССТ."
  else
  if DBL_Mest.LookupSource.DataSet.FieldByName('VOSST_POKRYT').AsString='НЕВОССТ.' then
...
var s:string;
begin
if CB_OTL.Checked then s:='убрать из отложенных нарядов'
    else s:='отложить';
if MessageBox(handle,PChar('Наряд '+s+'?'),'Диспетчер АВР',MB_OKCANCEL or MB_ICONINFORMATION)=ID_CANCEL
   then Key:=0;
  inherited;
