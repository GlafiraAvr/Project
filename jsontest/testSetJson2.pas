...
const fromatText = 'Проведение аварийных работ по устранению течи  %s по адресу %s';
...
public
 constructor create (http:ThttpSent;id:integer;date:TDateTime;tAtt:integer;adres:string;log:boolean=true);
...
constructor TSentToServ.create(http: ThttpSent; id: integer;
  date: TDateTime; tAtt: integer; adres: string;log:boolean);
begin
inherited create;
...
TAttach:=tatt;
if (tAtt=2) then
  self.id:=InttoStr(id)+'#k#'+FormatDateTime('yyyy',date)
else
  self.id:=InttoStr(id)+'#v#'+FormatDateTime('yyyy',date);
