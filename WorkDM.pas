    ...
    begin
      ...
      begin
        if not FieldByName( 'TZ_TRUDOZATRAT' ).IsNull then
          ...
          FieldByName( 'clcTRUDOZATRAT' ).AsString := 'не заданы';
      end else
        FieldByName( 'clcTRUDOZATRAT' ).AsString := 'на форме нар€да не задана местность пов.';
    end;
  end;
end;
...
begin
  ...
  begin
    Result := false;
    _ErrMsg :=  'Ќевозможно пересчитать трудозатраты,' + #13 +
